...
circle center(pt a, pt b, pt c) {
  b = b-a, c = c-a;
  assert(cross(b,c)!= 0);//no circle if A, B, C aligned
...
int inter_cl(circle c, line l, pair<pt, pt> &out) {
  ...
  if (h2 >= 0) { // line touches circle
    pt p=l.proj(c.c);
    pt h=l.v*sqrt(h2)/abs(l.v);//vec len h paralel->lne
    out = {p-h, p+h};
  }
...
int inter_cc(circle c1, circle c2, pair<pt, pt> &out) {
  pt d = c2.c-c1.c;
  double d2 = norm(d);//assert=concetri circles(equal)
...
int tangents(circle c1, circle c2, bool inner, 
  ...
  if (d2==0||h2<0) {assert(h2!=0);return 0;} //(equal)
  for (double s : {-1, 1}) {
