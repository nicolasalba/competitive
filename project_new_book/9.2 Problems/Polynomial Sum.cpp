...
1. Increase [a,b] by 1,2nd by 2,3rd by 3,and so on
2. Sum of [a,b]
cin >> nums[i],tree.update(i, { nums[i] })
For 1:tree.apply(l,r,{0,1});For 2:tree.query(l,r).sum*/
struct Node {ll sum = 0;};struct Func { ll add, ops; };
Node e() { return {0}; }; Func id() { return {0, 0}; }
...
struct lazytree {
  int n; vector<Node> nodes; vector<Func> lazy;
  void init(int nn) {
    n = nn; int size = 1; while (size<n){size*=2; }
    ll m = size * 2; nodes.assign(m, e());
    lazy.assign(m, id()); }
  ...
  Node query(int i, int sl, int sr, int l, int r) {
    push(i,sl,sr);
    if (l <= sl && sr <= r) { return nodes[i]; } 
    else if (sr < l || r < sl) { return e(); } 
    else { int mid = (sl + sr) >> 1;
